...
The initial run can be a bit slow, and I suspect that with a significant refactor can improve the time it takes to load into the tables, it can run faster. Also since Socrata holds a lot of cities data, I would like to add functionality to extract data from other cities collision datasets, such as D.C,Chicago and L.A
